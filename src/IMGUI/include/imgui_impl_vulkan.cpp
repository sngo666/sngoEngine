...
//  2024-02-14: *BREAKING CHANGE*: Moved RenderPass parameter from ImGui_ImplVulkan_Init() function to ImGui_ImplVulkan_InitInfo
//  structure. Not required when using dynamic rendering. 2024-02-12: *BREAKING CHANGE*: Dynamic rendering now require filling
//  PipelineRenderingCreateInfo structure. 2024-01-19: Vulkan: Fixed vkAcquireNextImageKHR() validation errors in
//  VulkanSDK 1.3.275 by allocating one extra semaphore than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory()
//  calls unnecessarily using full buffer size (#3957). Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added
//  MinAllocationSize field in ImGui_ImplVulkan_InitInfo to workaround zealous "best practice" validation layer. (#7189, #4238)
//  2024-01-03: Vulkan: Stopped creating command pools with VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't reset
//  them. 2023-11-29: Vulkan: Fixed mismatching allocator passed to vkCreateCommandPool() vs vkDestroyCommandPool(). (#7075)
//  2023-11-10: *BREAKING CHANGE*: Removed parameter from ImGui_ImplVulkan_CreateFontsTexture(): backend now creates its own
//  command-buffer to upload fonts.
//              *BREAKING CHANGE*: Removed ImGui_ImplVulkan_DestroyFontUploadObjects() which is now unecessary as we create and
//              destroy those objects in the backend. ImGui_ImplVulkan_CreateFontsTexture() is automatically called by NewFrame()
//              the first time. You can call ImGui_ImplVulkan_CreateFontsTexture() again to recreate the font atlas texture. Added
//              ImGui_ImplVulkan_DestroyFontsTexture() but you probably never need to call this.
//  2023-07-04: Vulkan: Added optional support for VK_KHR_dynamic_rendering. User needs to set init_info->UseDynamicRendering =
//  true and init_info->ColorAttachmentFormat. 2023-01-02: Vulkan: Fixed sampler passed to ImGui_ImplVulkan_AddTexture() not being
//  honored + removed a bunch of duplicate code. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        if (sizeof(ImTextureID) < sizeof(ImU64)) {
          // We don't support texture switches if ImTextureID hasn't been redefined to be 64-bit. Do a flaky check that other
  ...
  // We perform a call to vkCmdSetScissor() to set back a full viewport which is likely to fix things for 99% users but
...
bool ImGui_ImplVulkan_CreateFontsTexture() {
  ...
  // Copy to Image:
  {
    ...
    vkCmdPipelineBarrier(bd->FontCommandBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0,
...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator,
  ...
  if (bd->VulkanInitInfo.UseDynamicRendering) {
    IM_ASSERT(bd->VulkanInitInfo.PipelineRenderingCreateInfo.sType == VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(VkPhysicalDevice physical_device, VkSurfaceKHR surface,
  ...
  // Assuming that the default behavior is without setting this bit, there is no need for separate Swapchain image and image view
  // format Additionally several new color spaces were introduced with Vulkan Spec v1.0.40, hence we must make sure that a format
