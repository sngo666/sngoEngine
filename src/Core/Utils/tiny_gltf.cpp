...
static bool LoadExternalFile(std::vector<unsigned char>* out,
                             std::string* err,
                             std::string* warn,
...
static inline std::string WcharToUTF8(const std::wstring& wstr)
{
  ...
  WideCharToMultiByte(
      CP_UTF8, 0, wstr.data(), (int)wstr.size(), &str[0], (int)str.size(), nullptr, nullptr);
  return str;
}
#endif
...
bool GetFileSizeInBytes(size_t* filesize_out,
                        std::string* err,
                        const std::string& filepath,
                        void* userdata)
{
...
static bool ParseUnsignedProperty(size_t* ret,
                                  std::string* err,
                                  const detail::json& o,
                                  const std::string& property,
                                  const bool required,
...
static bool ParseIntegerArrayProperty(std::vector<int>* ret,
                                      std::string* err,
                                      const detail::json& o,
                                      const std::string& property,
                                      bool required,
...
static bool ParseStringProperty(std::string* ret,
                                std::string* err,
                                const detail::json& o,
                                const std::string& property,
                                bool required,
...
static bool ParseParameterProperty(Parameter* param,
                                   std::string* err,
                                   const detail::json& o,
...
static bool ParseExtrasAndExtensions(GltfType* target,
                                     std::string* err,
                                     const detail::json& o,
...
static bool ParseAsset(Asset* asset,
                       std::string* err,
                       const detail::json& o,
...
static bool ParseTexture(Texture* texture,
                         std::string* err,
                         const detail::json& o,
                         bool store_original_json_for_extras_and_extensions,
                         const std::string& basedir)
...
static bool ParseTextureInfo(TextureInfo* texinfo,
                             std::string* err,
                             const detail::json& o,
                             bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseNormalTextureInfo(NormalTextureInfo* texinfo,
                                   std::string* err,
                                   const detail::json& o,
                                   bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseBufferView(BufferView* bufferView,
                            std::string* err,
                            const detail::json& o,
                            bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseSparseAccessor(Accessor::Sparse* sparse,
                                std::string* err,
                                const detail::json& o,
                                bool store_original_json_for_extras_and_extensions)
{
  ...
  if (!ParseIntegerProperty(
          &indices_buffer_view, err, indices_obj, "bufferView", true, "SparseAccessor"))
    {
      return false;
    }
  ParseUnsignedProperty(&indices_byte_offset, err, indices_obj, "byteOffset", false);
  if (!ParseIntegerProperty(
          &component_type, err, indices_obj, "componentType", true, "SparseAccessor"))
    {
      return false;
    }
  ...
  if (!ParseIntegerProperty(
          &values_buffer_view, err, values_obj, "bufferView", true, "SparseAccessor"))
    {
      return false;
    }
  ...
  ParseExtrasAndExtensions(
      &sparse->indices, err, indices_obj, store_original_json_for_extras_and_extensions);

  sparse->values.bufferView = values_buffer_view;
  sparse->values.byteOffset = values_byte_offset;
  ParseExtrasAndExtensions(
      &sparse->values, err, values_obj, store_original_json_for_extras_and_extensions);

  return true;
}

static bool ParseAccessor(Accessor* accessor,
                          std::string* err,
                          const detail::json& o,
                          bool store_original_json_for_extras_and_extensions)
{
  ...
  if (detail::FindMember(o, "sparse", iterator))
    {
      ...
      return ParseSparseAccessor(&accessor->sparse,
                                 err,
...
static void DecodeIndexBuffer(draco::Mesh* mesh,
      ...
      for (draco::FaceIndex f(0); f < mesh->num_faces(); ++f)
        {
          ...
          if (componentSize == 2)
            ...
            {
              uint8_t indices[3] = {
                  (uint8_t)face[0].value(), (uint8_t)face[1].value(), (uint8_t)face[2].value()};
              memcpy(outBuffer.data() + f.value() * faceStride, &indices[0], faceStride);
            }
        }
    }
...
static bool GetAttributeForAllPoints(uint32_t componentType,
                                     draco::Mesh* mesh,
                                     const draco::PointAttribute* pAttribute,
                                     std::vector<uint8_t>& outBuffer)
{
...
static bool ParseDracoExtension(Primitive* primitive,
                                Model* model,
                                std::string* err,
                                std::string* warn,
...
static bool ParsePrimitive(Primitive* primitive,
                           Model* model,
                           std::string* err,
                           std::string* warn,
                           const detail::json& o,
...
static bool ParseMesh(Mesh* mesh,
                      Model* model,
                      std::string* err,
                      std::string* warn,
                      const detail::json& o,
...
static bool ParseNode(Node* node,
                      std::string* err,
                      const detail::json& o,
                      bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseScene(Scene* scene,
                       std::string* err,
                       const detail::json& o,
                       bool store_original_json_for_extras_and_extensions)
{
                                      ...
                                      bool store_original_json_for_extras_and_extensions)
{
...

  {
    ...
    if (detail::FindMember(o, "baseColorTexture", it))
      {
        ParseTextureInfo(&pbr->baseColorTexture,
                         err,
                         detail::GetValue(it),
                         store_original_json_for_extras_and_extensions);
      }
...

  {
    ...
    if (detail::FindMember(o, "metallicRoughnessTexture", it))
      {
        ParseTextureInfo(&pbr->metallicRoughnessTexture,
                         err,
                         detail::GetValue(it),
                         store_original_json_for_extras_and_extensions);
      }
...
static bool ParseMaterial(Material* material,
                          std::string* err,
                          std::string* warn,
                          const detail::json& o,
                          bool store_original_json_for_extras_and_extensions,
                          ParseStrictness strictness)
{
...

  {
    ...
    if (detail::FindMember(o, "pbrMetallicRoughness", it))
      {
        ParsePbrMetallicRoughness(&material->pbrMetallicRoughness,
                                  err,
                                  detail::GetValue(it),
                                  store_original_json_for_extras_and_extensions);
      }
...

  {
    ...
    if (detail::FindMember(o, "normalTexture", it))
      {
        ParseNormalTextureInfo(&material->normalTexture,
                               err,
                               detail::GetValue(it),
                               store_original_json_for_extras_and_extensions);
      }
...

  {
    ...
    if (detail::FindMember(o, "occlusionTexture", it))
      {
        ParseOcclusionTextureInfo(&material->occlusionTexture,
                                  err,
                                  detail::GetValue(it),
                                  store_original_json_for_extras_and_extensions);
      }
...

  {
    ...
    if (detail::FindMember(o, "emissiveTexture", it))
      {
        ParseTextureInfo(&material->emissiveTexture,
                         err,
                         detail::GetValue(it),
                         store_original_json_for_extras_and_extensions);
      }
              ...
              for (; itVal != itValEnd; ++itVal)
                {
                  ...
                  if (ParseParameterProperty(
                          &param, err, values_object, detail::GetKey(itVal), false))
                    {
...
static bool ParseAnimationChannel(AnimationChannel* channel,
                                  std::string* err,
                                  const detail::json& o,
                                  bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseAnimation(Animation* animation,
                           std::string* err,
                           const detail::json& o,
                           bool store_original_json_for_extras_and_extensions)
{
        ...
        for (detail::json_const_array_iterator i = detail::ArrayBegin(detail::GetValue(channelsIt));
             ...
             ++i)
          {
            ...
            if (ParseAnimationChannel(
                    &channel, err, *i, store_original_json_for_extras_and_extensions))
              {
...
static bool ParseSampler(Sampler* sampler,
                         std::string* err,
                         const detail::json& o,
                         bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseSkin(Skin* skin,
                      std::string* err,
                      const detail::json& o,
                      bool store_original_json_for_extras_and_extensions)
{
...
static bool ParsePerspectiveCamera(PerspectiveCamera* camera,
                                   std::string* err,
                                   const detail::json& o,
                                   bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseCamera(Camera* camera,
                        std::string* err,
                        const detail::json& o,
                        bool store_original_json_for_extras_and_extensions)
{
  ...
  if (camera->type.compare("orthographic") == 0)
    {
      ...
      if (!ParseOrthographicCamera(
              &camera->orthographic, err, v, store_original_json_for_extras_and_extensions))
        {
          return false;
        }
    }
  else if (camera->type.compare("perspective") == 0)
    {
      ...
      if (!ParsePerspectiveCamera(
              &camera->perspective, err, v, store_original_json_for_extras_and_extensions))
        {
          return false;
        }
    }
...
static bool ParseLight(Light* light,
                       std::string* err,
                       const detail::json& o,
                       bool store_original_json_for_extras_and_extensions)
{
...
static bool ParseAudioEmitter(AudioEmitter* emitter,
                              std::string* err,
                              const detail::json& o,
                              bool store_original_json_for_extras_and_extensions)
{
  ...
  if (emitter->type == "positional")
    {
      ...
      if (!ParsePositionalEmitter(
              &emitter->positional, err, v, store_original_json_for_extras_and_extensions))
        {
          return false;
        }
    }
...
static bool ParseAudioSource(AudioSource* source,
                             std::string* err,
                             const detail::json& o,
                             bool store_original_json_for_extras_and_extensions)
{
                              ...
                              unsigned int check_sections)
{
              ...
              if (bufferView < 0)
              ...
              else if (size_t(bufferView) >= model->bufferViews.size())
                {
                  if (err)
                    {
                      (*err) +=
                          "accessor[" + std::to_string(primitive.indices) + "] invalid bufferView";
                    }
                  return false;
                }
              else
  ...
  // 10. Parse Material
  {
    bool success = ForEachInArray(v, "materials", [&](const detail::json& o) {
      ...
      if (!ParseMaterial(
              &material, err, warn, o, store_original_json_for_extras_and_extensions_, strictness_))
        {
          return false;
        }
...
bool TinyGLTF::LoadASCIIFromString(Model* model,
                                   std::string* err,
                                   std::string* warn,
                                   const char* str,
                                   unsigned int length,
...
bool TinyGLTF::LoadASCIIFromFile(Model* model,
                                 std::string* err,
                                 std::string* warn,
                                 const std::string& filename,
                                 unsigned int check_sections)
...
bool TinyGLTF::LoadBinaryFromMemory(Model* model,
                                    std::string* err,
                                    std::string* warn,
                                    const unsigned char* bytes,
                                    unsigned int size,
                                    ...
                                    unsigned int check_sections)
{
  ...
  if ((header_and_json_size > uint64_t(size)) || (chunk0_length < 1) || (length > size)
      || (header_and_json_size > uint64_t(length)) || (chunk0_format != 0x4E4F534A))
  ...
  if (header_and_json_size == uint64_t(length))
  ...
  else
    {
      ...
      if (chunk1_length == 0)
        ...
        {
          // When BIN chunk size is not zero, at least Chunk1 should have 12 bytes(8 bytes(header) +
          // 4 bytes(bin payload could be 1~3 bytes, but need to be aligned to 4 bytes)
...
bool TinyGLTF::LoadBinaryFromFile(Model* model,
                                  std::string* err,
                                  std::string* warn,
                                  const std::string& filename,
                                  unsigned int check_sections)
...
void JsonAddMember(detail::json& o, const char* key, detail::json&& value)
{
  ...
  if (detail::FindMember(o, key, it))
    ...
    {
      o.AddMember(
          detail::json(key, detail::GetAllocator()), std::move(value), detail::GetAllocator());
    }
#else
  ...
  if ((accessor.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT)
  ...
  else
    {
      ...
      // Assume int value is within [-2**31-1, 2**31-1]
      {
        ...
        std::transform(accessor.minValues.begin(),
      ...
      {
        std::vector<int> values;
        std::transform(accessor.maxValues.begin(),
...
static void SerializeGltfBufferBin(const Buffer& buffer,
                                   detail::json& o,
...
static bool SerializeGltfBuffer(const Buffer& buffer,
                                detail::json& o,
                                const std::string& binFilename,
  ...
  if (model->lights.size())
    {
      ...
      // Also add "KHR_lights_punctual" to `extensionsUsed`
      {
        auto has_khr_lights_punctual =
            std::find_if(extensionsUsed.begin(), extensionsUsed.end(), [](const std::string& s) {
...
bool TinyGLTF::WriteGltfSceneToStream(const Model* model,
                                      std::ostream& stream,
      ...
      for (unsigned int i = 0; i < model->images.size(); ++i)
        {
          ...
          // we
          std::string uri;
          if (!UpdateImageObject(model->images[i],
                                 dummystring,
                                 int(i),
                                 true,
                                 &uri_cb,
                      ...
                      for (const std::string& usedName : usedFilenames)
                        {
                          ...
                          inUse = true;
                          binFilename =
                              defaultBinFilename + std::to_string(numUsed++) + defaultBinFileExt;
                          break;
                        }
                    }
